...
for bacteria_file in bacteria_names:
    g_file = f"/srv/scratch/AMR/Variant_GPA_Kmer_uncompressed/{bacteria_file}_train_genotype_vargpakmer.tsv"
    ...
    with open(f"/srv/scratch/AMR/XGB_Cleaned_KMER_IR/{bacteria_file}/seed_42_testsize_0.2_resampling_holdout_XGBOOST_model.sav", 'rb') as trained_model_file:
    ...
    feature_importance_ans = open(f"feature_importance_{bacteria_file}.txt", 'w')

    for i in sorted_importances:
        if i[1] <= 0:
            break
        else:
            print(i, file = feature_importance_ans)
